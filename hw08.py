...
def run_func(op_code_node):
    ...
    def plus(node):
        if (node.value.next.type is TokenType.LIST):
            temp = run_expr(node.value.next)
            temp2 = node.value.next.next
            node.value.next = temp
        ...
        result = strip_quote(l_node)
        value1 = result.value
        if (result.next.type is TokenType.LIST):
            temp = run_expr(result.next)
            temp2 = result.next.next
            result.next = temp
            if (temp2 is not None):
                temp.next = temp2
        value2 = result.next.value
    ...
    def run_cond(node):
        ...
        if (node.value.type is TokenType.LIST):
            temp = run_expr(node.value)
            temp.next = node.value.next
            temp = create_new_quote_list(temp)
            temp = strip_quote(temp)
            temp.next = node.next
            node = temp
    ...
    def lambda_1(node) : #add lambda_1 method by SJH
        ...
        if input_value is not None:
            if input_func.next is not None:
                while input_func.next is not None:
                    input_temp = {}
                ...
                symbol_table.pop(node.value.next.next.value.next.value)
    ...
    table['='] = eq
    table['cond'] = cond
    table['lambda'] = lambda_1#add by SJH
